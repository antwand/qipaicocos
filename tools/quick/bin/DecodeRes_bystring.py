...
def checkFileExt(path):
    binExt = [".zip", ".jpg", ".jpeg", ".png", ".pvr", ".ccz", ".bmp",".luac"]
...
def copyFile(src, dest, encodeSign, encodeKey):
    ...
    if checkFileExt(src):
        # xxtea encoder
        #encodeSignLen = len(encodeSign)
        #buffLen = len(buff)
        #newbuff = buff[4,20]
        buff = buff.replace(encodeSign,'')
        buff = xxtea.decrypt(buff, encodeKey)
...
def encodeRes(projectDir, encodeSign, encodeKey):
    ...
    resBKPath = joinDir(projectDir, "res_bk_de")
